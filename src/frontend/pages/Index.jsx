...
import {
  ...
  captureGoogleAdsIds,
  getGoogleAdsIdsForCheckout,
} from "../../utils/googleAdsTracking";
import {
  MAIN_SITE_URL,
  clearBookingClientStorage,
  navigateToMainSite,
} from "../../utils/crossDomainNavigation";
...
const TRIPADVISOR_REVIEW_LINK =
  "https://www.tripadvisor.in/Attraction_Review-g190730-d21508626-Reviews-Fly_Away_Ballooning-Taunton_Somerset_England.html";
const SIDE_FAQ_ITEMS = [
  {
    id: "faq-wear",
    title: "What should I wear & bring?",
    content:
      "Wear closed-toe footwear, weather-appropriate layers and avoid loose scarves during flight. Bags are not allowed in the basket and should stay in the ground crew vehicle.",
  },
  {
    id: "faq-age",
    title: "Is there an age limit?",
    content:
      "There is no maximum age limit, but children under 7 years old cannot fly due to safety regulations.",
  },
  {
    id: "faq-weight",
    title: "Is there a weight limit?",
    content:
      "Shared flights have a passenger weight guideline of 18 stone (115 kg). A supplement may apply where a second seat is required for safe weight-to-volume operation.",
  },
  {
    id: "faq-weather-cancel",
    title: "What happens if my flight is cancelled?",
    content:
      "You will receive SMS/email updates and can rebook from your customer portal. Weather-refund options apply only when selected at checkout and subject to terms.",
  },
  {
    id: "faq-flight-times",
    title: "What times do we fly?",
    content:
      "Flights run at sunrise or sunset windows. Exact meeting time changes seasonally and is shown in the live booking calendar.",
  },
  {
    id: "faq-redeem",
    title: "How do I redeem a gift voucher?",
    content:
      'Choose "Redeem Voucher" in booking, enter your voucher reference, then select location/date/time and complete passenger details.',
  },
  {
    id: "faq-validity",
    title: "How long is my voucher valid for?",
    content:
      "Any Day Shared vouchers are valid for 24 months. Other vouchers are typically valid for 18 months, with extension rules based on genuine weather attempts.",
  },
  {
    id: "faq-refund",
    title: "Can I get a refund?",
    content:
      "Refunds are available only for bookings that include the refundable option and only under weather-cancellation conditions, according to terms.",
  },
];
...
const HOTEL_MANUAL_BOOKING_GUIDE = [
  ...
  },
  {
    ...
    items: [
      {
        title: "Shared Flights",
        items: ["Guests are refunded £220 per person"],
      },
      {
        title: "Private Flights",
        items: ["Refund issued minus 10%"],
      },
    ],
  },
  {
...
const Index = () => {
  ...
  const [chooseFlightType, setChooseFlightType] = useState({
    type: "",
    passengerCount: "",
    price: "",
  });
  ...
  const [passengerData, setPassengerData] = useState([
    { firstName: "", lastName: "", weight: "", weatherRefund: false },
  ]);
  const [weatherRefund, setWeatherRefund] = useState(false);
  const [privateCharterWeatherRefund, setPrivateCharterWeatherRefund] =
    useState(false);
  ...
  const [hasLiveAvailabilityResponse, setHasLiveAvailabilityResponse] =
    useState(false);
  ...
  const [isLiveAvailabilityLoading, setIsLiveAvailabilityLoading] =
    useState(false);
  ...
  useEffect(() => {
    const allLoaded =
      !accordionLoadingStates.isAccordionLoading &&
  ...
  const manualBookingRouteProfile =
    MANUAL_BOOKING_ROUTE_PROFILES[normalizedPath] || null;
  const dedicatedBookingDefaults =
    manualBookingRouteProfile?.bookingDefaults || null;
  const hiddenDedicatedSectionIds =
    manualBookingRouteProfile?.hiddenSectionIds || EMPTY_ARRAY;
  const isDedicatedManualBookingFlow = !!manualBookingRouteProfile;
  const manualBookingRequested = ["1", "true", "yes"].includes(
    (manualBookingParams.get("manualBooking") || "").toLowerCase(),
  );
  const manualBookingToken =
  ...
  const hasInvalidManualBookingLink =
    requiresManualBookingToken && !manualBookingToken;
  const manualBookingAccessStorageKey = buildManualBookingAccessStorageKey(
    normalizedPath,
    manualBookingToken,
  );
  const shouldShowManualBookingBanner =
    manualBookingRequested || !!manualBookingRouteProfile?.showBanner;
  const shouldUseCompactVoucherCards =
      ...
      try {
        const storedToken =
          window.localStorage.getItem(
            buildManualBookingAccessStorageKey(
              normalizedPath,
              manualBookingToken,
            ),
          ) || "";
  ...
  const buildDefaultPassengerData = useCallback(
    (passengerCountOverride = null) => {
      const passengerCount =
        passengerCountOverride ?? dedicatedBookingDefaults?.passengers ?? 1;
      const normalizedPassengerCount = Math.max(
        parseInt(passengerCount, 10) || 1,
        1,
      );
      ...
      return Array.from({ length: normalizedPassengerCount }, (_, index) => ({
        ...
        weight: "",
        phone:
          index === 0 ? dedicatedBookingDefaults?.passengerPhone || "" : "",
        countryCode: dedicatedBookingDefaults?.passengerCountryCode || "+44",
        email:
          index === 0 ? dedicatedBookingDefaults?.passengerEmail || "" : "",
  ...
  const isManualBookingContactValid = useCallback(
    (details) => {
  ...
  const authorizeManualBookingAccess = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        const response = await axios.post(
          `${API_BASE_URL}/api/authorize-manual-booking`,
          {
            manualBookingToken,
          },
        );
        ...
        if (!response.data?.success || !accessToken) {
          throw new Error(
            response.data?.message ||
              "Manual booking authorisation could not be refreshed.",
          );
        }
        ...
        if (typeof window !== "undefined") {
          try {
            ...
            console.warn(
              "Could not persist manual booking access token:",
              storageError,
            );
          }
        }
    ...
    if (typeof window !== "undefined") {
      try {
        const storedAccessToken =
          window.localStorage.getItem(manualBookingAccessStorageKey) || "";
        const storedAccessTokenExpiry =
          getTokenExpiryTimestamp(storedAccessToken);
        const hasUsableStoredAccessToken =
          !!storedAccessToken &&
          (!storedAccessTokenExpiry || storedAccessTokenExpiry > Date.now());
        ...
        if (
          ...
          storedAccessToken !== manualBookingAccessTokenRef.current
        ) {
          setManualBookingAccessToken(storedAccessToken);
        }
      } catch (storageError) {
        console.warn(
          "Could not read manual booking access token from storage:",
          storageError,
        );
      }
    }
  ...
  useEffect(() => {
    ...
    const nextPassengerData = buildDefaultPassengerData(
      dedicatedBookingDefaults.passengers,
    );

    setChooseLocation((prev) =>
      prev === dedicatedBookingDefaults.location
        ? prev
        : dedicatedBookingDefaults.location,
    );
    ...
    setSelectedVoucherType((prev) => {
      if (
        ...
        String(prev?.quantity || "") ===
          String(nextVoucherType?.quantity || "") &&
    ...
    setPassengerData((prev) => {
      ...
      return prev.map((passenger, index) => {
        if (index !== 0) {
          return {
            ...
            countryCode:
              ...
              nextPassengerData[index]?.countryCode ||
              "+44",
            weatherRefund: dedicatedBookingDefaults.weatherRefundable
              ? true
        ...
        return {
          ...
          countryCode:
            passenger?.countryCode ||
            nextPassengerData[0]?.countryCode ||
            "+44",
    ...
    axios
      .post(`${API_BASE_URL}/api/getActivityId`, {
      ...
      .catch((error) => {
        if (!cancelled) {
          console.error(
            "Error preloading dedicated manual booking activity:",
            error,
          );
        }
      });
  ...
  useEffect(() => {
    if (token) {
      ...
      axios
        .get(`${API_BASE_URL}/api/customer-portal-booking/${token}`)
        .then((response) => {
          if (response.data.success) {
            setPortalBookingData(response.data.data);
          } else {
            setPortalError(
              response.data.message || "Failed to load booking data.",
            );
          }
        })
        .catch((err) => {
  ...
  const filterHiddenSections = useCallback(
    (sections) => {
      ...
      return sections.filter(
        (sectionId) => !hiddenDedicatedSectionIds.includes(sectionId),
      );
    },
    [hiddenDedicatedSectionIds],
  );

  const withOptionalAdditionalInfoStep = (sections) =>
    shouldHideAdditionalInfoSection
      ? sections
      : [...sections, "additional-info"];
  const withOptionalAddOnStep = (sections) =>
    shouldHideAddOns ? sections : [...sections, "add-on"];
  const withOptionalHotelFlowSteps = (sections) =>
    withOptionalAddOnStep(withOptionalAdditionalInfoStep(sections));

  // Define progress sections based on activity type
  const progressSections =
    activitySelect === "Book Flight"
      ? filterHiddenSections(
          withOptionalHotelFlowSteps([
            "activity",
            "location",
            "experience",
            "voucher-type",
        ...
        : activitySelect === "Buy Gift"
          ...
          : activitySelect === "Redeem Voucher"
            ? filterHiddenSections(
              ...
              ) // Experience removed from progress bar for Redeem Voucher
            : [];
  ...
  useEffect(() => {
    ...
    if (selectedDate && selectedTime) {
      ...
      setCountdownSeconds((prev) =>
        prev === null || prev === undefined ? 300 : prev,
      );
  ...
  useEffect(() => {
    const holdAvailability = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          `${API_BASE_URL}/api/holdAvailability`,
          {
  ...
  useEffect(() => {
    if (countdownSeconds === 0) {
      ...
      if (holdActive) {
        axios
          .post(`${API_BASE_URL}/api/releaseHold`, {
            sessionId: sessionId,
          })
          .then(() => {
            setHoldActive(false);
  ...
  const showBookingCompletionModal = ({
    type,
    responseData,
    manualBooking = false,
  }) => {
  ...
  const refetchAvailabilities = useCallback(
    async (skipDebounce = false) => {
      if (chooseLocation && activityId) {
        ...
        if (
          availabilitiesRequestControllerRef.current &&
          lastAvailabilitiesRequestRef.current !== currentRequestKey
        ) {
    ...
    if (
      ...
      if (!currentCalendarDate) {
        ...
        if (
          ...
          lastAvailabilitiesRequestRef.current === expectedRequestKey
        ) {
          return;
        }
  ...
  const [passengerTermsModalOpen, setPassengerTermsModalOpen] =
    React.useState(false);
  const [passengerTermsContent, setPassengerTermsContent] = React.useState("");
  const [passengerTermsContentCache, setPassengerTermsContentCache] =
    React.useState("");
  const [passengerTermsShownForJourney, setPassengerTermsShownForJourney] =
    React.useState("");
  ...
  const fetchPassengerTermsForJourney = async (
    journeyLabelRaw,
    { openModal = true, preferCache = true } = {},
  ) => {
    try {
      ...
      try {
        ...
        console.warn(
          "Passenger terms journey endpoint failed, will try fallback",
          e,
        );
      }
      ...
      if (!combined) {
        try {
          ...
          if (respAll?.data?.success && Array.isArray(respAll.data.data)) {
            ...
            const matched = respAll.data.data.filter((t) =>
              safeParse(t.journey_types).includes(journeyLabel),
            );
            combined = matched
              .map((t) => (t && t.content ? String(t.content) : ""))
              .filter(Boolean)
  ...
  const validateVoucherCode = async (codeToValidate = null) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/voucher-codes/validate`,
        {
          code: code.trim(),
      ...
      if (response.data.success) {
        ...
        if (expiresDate) {
          ...
          if (now > expires) {
            ...
            alert(
              `This voucher has expired. The expiration date (${expiresFormatted}) has passed. Expired vouchers cannot be used for redemption.`,
            );
            return;
          }
        }
        ...
        const voucherAmount =
          ...
          response.data.data.voucher_amount ||
          0;
        if (voucherAmount > 0) {
          setChooseFlightType((prev) => ({
            ...prev,
        ...
        if (activitySelect === "Redeem Voucher" && response.data.data) {
          ...
          const resolvedPassengerCount = parseInt(
            voucherInfo.numberOfPassengers,
            10,
          );
          // Set flight type from voucher experience_type
          if (voucherInfo.experience_type) {
            const voucherPrice =
              voucherInfo.final_amount || voucherInfo.voucher_amount || 0;
            setChooseFlightType({
              type: voucherInfo.experience_type,
              passengerCount:
                !Number.isNaN(resolvedPassengerCount) &&
                resolvedPassengerCount > 0
          ...
          if (mappedVoucherType) {
            ...
            if (normalizedType === "Weekday Morning Flight") {
              ...
              normalizedType === "Any Day" ||
              normalizedType === "Anytime"
            ) {
            ...
            if (
              ...
              if (
                ...
                normalizedType = "Any Day Flight";
                console.warn(
                  "Voucher type not in valid list, defaulting to Any Day Flight:",
                  mappedVoucherType,
                );
              }
            ...
            const selectedVoucherTypeQuantity =
              !Number.isNaN(resolvedPassengerCount) &&
              resolvedPassengerCount > 0
        ...
        const errorMessage =
          response.data.message || "Voucher code validation failed";
  ...
  const isAdditionalInfoStepComplete =
    shouldHideAdditionalInfoSection || isAdditionalInfoValid(additionalInfo);
  ...
  const hasDedicatedManualBookingRequirements =
    !isDedicatedManualBookingFlow || manualBookingContactComplete;
  ...
  if (activitySelect === "Book Flight") {
    ...
    const voucherTypePrice = (() => {
      ...
      if (selectedVoucherType?.priceUnit === "total")
        return selectedVoucherType?.price ?? 0;
      const basePrice =
        selectedVoucherType?.basePrice ?? selectedVoucherType?.price ?? 0;
      const quantity = selectedVoucherType?.quantity ?? 1;
      return basePrice * quantity;
    })();
    const addOnPrice =
      chooseAddOn?.reduce((sum, addon) => sum + (addon.price || 0), 0) || 0;
    ...
    const weatherRefundPrice = isPrivateCharter
      ? privateCharterWeatherRefund
        ? voucherTypePrice * 0.1
        : 0
      : Array.isArray(passengerData)
        ? passengerData.reduce(
            (sum, p) => sum + (p && p.weatherRefund ? 47.5 : 0),
            0,
          )
        : 0;
    totalPrice =
  ...
  const getSectionSequence = (
    ...
    const isPassengerInfoComplete =
      Array.isArray(currentPassengerData) &&
      currentPassengerData.every((passenger, index) => {
        const isFirstPassenger = index === 0;
        const basicInfoValid =
          passenger.firstName &&
          passenger.firstName.trim() !== "" &&
  ...
  const handleSectionCompletion = (completedSectionId) => {
    ...
    const isLoadingCheck =
      isLiveAvailabilityLoading || isLiveAvailabilityLoadingRef.current;
    ...
    setTimeout(() => {
      ...
      if (isFreshStart) {
        ...
        if (selectedDate && selectedTime)
          completedSections.push("live-availability");
        // Use proper passenger info validation based on activity type
        const passengerComplete = isGiftVoucher
          ? isBuyGiftPassengerComplete
          : isPassengerInfoComplete;
        ...
        if (
          ...
          completedSections.push("additional-info");
        if (
        ...
        if (chooseAddOn && chooseAddOn.length > 0)
          completedSections.push("add-on");
      ...
      if (activitySelect === "Redeem Voucher") {
        ...
        // After live-availability: skip experience and go directly to passenger-info
          ...
          completedSectionId === "live-availability" &&
      ...
      if (nextSection) {
        ...
        if (nextSection === "live-availability") {
          ...
          if (isShopifyFlow && isVoucherTypeStart) {
            ...
            const hasRequiredState =
              ...
              selectedVoucherType?.title;
            const termsReady =
              voucherTermsLoadedRef.current || !voucherTermsLoading; // Terms loaded or not loading
            const accordionsReady =
              !accordionLoadingStates.isAccordionLoading &&
              accordionLoadedRef.current; // All accordions loaded
            ...
            const isVoucherTypeCompleted =
              completedSectionId === "voucher-type" &&
              selectedVoucherType?.title;
            const shouldOpenAfterTermsAcceptance =
              isTermsAccepted &&
            ...
            if (
              ...
              (!hasRequiredState || !termsReady || !accordionsReady)
            ) {
              ...
              const waitForState = () => {
                ...
                if (readyNow || (elapsed >= 8000 && hasActivityId)) {
                  ...
                  if (elapsed >= 8000 && !hasActivityId) {
                    ...
                    if (elapsed < 15000) {
                      liveAvailabilityGuardTimerRef.current = setTimeout(
                        waitForState,
                        200,
                      );
                    } else {
                      ...
                      console.error(
                        "[ShopifyDebug] Guard: Absolute timeout reached, activityId still null - cannot open section",
                      );
                        ...
                        .then((data) => {
                          if (data && data.length > 0) {
                            ...
                            const pollForUpdate = () => {
                              pollCount++;
                              if (
                                availabilitiesRef.current &&
                                availabilitiesRef.current.length > 0
                              ) {
                                // Now safe to open
              ...
              if (!activityIdRef.current) {
                ...
                const pollForActivityId = () => {
                  ...
                  if (activityIdRef.current) {
                    ...
                    safeAutoOpenAccordion(
                      "live-availability",
                      "pollForActivityId-timeout",
                    );
                    setTimeout(
                      () => setIsLiveAvailabilityLoadingSync(false),
                      500,
        ...
        const isLoadingCheckForOpen =
          isLiveAvailabilityLoading || isLiveAvailabilityLoadingRef.current;
        const justOpenedLiveAvailability =
          nextSection === "live-availability" &&
          (activeAccordion !== "live-availability" || isLoadingCheckForOpen);

        if (
          (activeAccordion === "live-availability" ||
            justOpenedLiveAvailability) &&
          nextSection !== "live-availability"
        ) {
    ...
    }, 140); // Keep the transition responsive without abrupt jumps
  };
  ...
  // Otomatik akışların (Shopify guard vs.) accordion açarken
  // Passenger Information'dan Live Availability'e geri sıçrama yapmaması için güvenli setter
  const safeAutoOpenAccordion = React.useCallback(
    (sectionId) => {
      const capturedActive = activeAccordion;
      const currentActive = activeAccordionRef.current;

      // Block ANY auto-open to Live Availability if Passenger Information
      // is either the captured or current accordion (protects against stale closures)
      if (
        sectionId === "live-availability" &&
        (capturedActive === "passenger-info" ||
          currentActive === "passenger-info")
      ) {
        return;
      }
  ...
  const getAccordionState = (sectionId) => {
    ...
    if (
      (chooseFlightType?.type || hasExperienceFromShopify) &&
      activitySelect !== "Redeem Voucher"
    ) {
      completedSections.push("experience");
    }
    ...
    const sectionsToCheck =
      activitySelect === "Redeem Voucher"
        ...
        : previousSections;
    const allPreviousCompleted = sectionsToCheck.every((section) =>
  ...
  const completeManualBookingFlow = async ({
    type,
    bookingData = null,
    voucherData = null,
    totalPrice,
    ...
    const resolvedAccessToken =
      manualBookingAccessTokenRef.current ||
      (await authorizeManualBookingAccess());
    if (!resolvedAccessToken && manualBookingAuthError) {
      throw new Error(manualBookingAuthError);
    }

    try {
      const manualSessionRes = await axios.post(
        `${API_BASE_URL}/api/create-manual-session`,
        {
          totalPrice,
          bookingData,
          voucherData,
      ...
      if (
        !manualSessionRes.data?.success ||
        !manualSessionRes.data?.sessionId
      ) {
        throw new Error(
          manualSessionRes.data?.message ||
            "Manual booking session could not be created.",
        );
      }

      const response = await axios.post(
        `${API_BASE_URL}/api/createBookingFromSession`,
        {
      ...
      if (!response.data?.success) {
        throw new Error(
          response.data?.message || "Manual booking could not be completed.",
        );
      }
  ...
  const handleBookData = async () => {
    if (hasInvalidManualBookingLink) {
      alert(
        "This manual booking link is incomplete. Please reopen the booking flow from the admin system.",
      );
      return;
    }

    if (
      requiresManualBookingToken &&
      !manualBookingAccessTokenRef.current &&
      manualBookingAuthError
    ) {
    ...
    const submissionAdditionalInfo =
      buildSubmissionAdditionalInfo(additionalInfo);
    ...
    const resolveVoucherPassengerCount = () => {
      ...
      if (Array.isArray(passengerData) && passengerData.length > 0) {
        const filled = passengerData.filter(
          (p) => p && (p.firstName || p.lastName || p.email || p.phone),
        ).length;
    ...
    if (isFlightVoucher || isGiftVoucher) {
      ...
      try {
        const testResponse = await axios.post(
          `${API_BASE_URL}/api/testAdditionalInfo`,
          {
    ...
    if (isRedeemVoucher) {
      ...
      const voucherData = {
        ...
        additionalInfo: submissionAdditionalInfo, // Add additional information data
        add_to_booking_items:
          chooseAddOn && chooseAddOn.length > 0 ? chooseAddOn : null, // Add add to booking items
      ...
      const privateWeatherRefundSelected =
        chooseFlightType?.type === "Private Charter" &&
        privateCharterWeatherRefund;
      const redeemBookingPassengerDataWithPhoneCode = passengerData.map(
        (p) => ({
          ...p,
          phone:
            p.countryCode && p.phone
              ? `${p.countryCode}${p.phone}`.trim()
              : p.phone || "",
          weatherRefund: privateWeatherRefundSelected
            ? true
            : !!p.weatherRefund,
        }),
      );
  ...
  useEffect(() => {
    ...
    if (chooseLocation && activeAccordion === "live-availability") {
      ...
      const isVoucherTypeStartCheck =
        paramsForCheck.get("startAt") === "voucher-type";
      ...
      if (isShopifyFlow) {
        ...
        const fetchAvailabilities = async () => {
          try {
            ...
            // IMPORTANT: For "Private Charter" we intentionally do NOT send a flightType filter.
            // The activityId for Private Charter is already specific enough and some
            // production environments may not recognize the newer "Private" value,
            // so omitting the filter keeps behaviour consistent with the legacy API.
            if (
              chooseFlightType &&
              chooseFlightType.type &&
              chooseFlightType.type !== "Private Charter"
  ...
  useEffect(() => {
    try {
      ...
      const snapshot = {
        ...
        availabilitiesCount: Array.isArray(availabilities)
      ...
      if (!availabilities || availabilities.length === 0) {
        console.warn(
          "[DeepLinkDebug] Live Availability EMPTY for deep link URL",
          {
            url: window.location.href,
            chooseLocation,
            activityId,
  ...
  useEffect(() => {
    ...
    const shouldFetch =
      (activeAccordion === "live-availability" || isVoucherTypeStart) &&
      hasRequiredState;
  ...
  useEffect(() => {
    if (
      ...
      (activitySelect === "Flight Voucher" || activitySelect === "Book Flight")
    ) {
      setChooseFlightType((prev) => ({
        ...prev,
    ...
    if (activitySelect !== null) {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const source = params.get("source");
          const startAt = params.get("startAt");
          shouldSkipAutoOpenInTimeout =
            source === "shopify" && startAt === "voucher-type";
        } catch {}
        ...
        if (sequence.length > 1) {
          ...
          if (
            firstSectionAfterActivity === "experience" &&
            (shopifyStartAtVoucher || shouldSkipAutoOpenInTimeout)
          ) {
            // Find next section after experience
            const experienceIndex = sequence.indexOf("experience");
            if (
              experienceIndex !== -1 &&
              sequence.length > experienceIndex + 1
            ) {
  ...
  React.useEffect(() => {
    // Don't run this effect if recipient-details or passenger-info accordion is open and user is typing
    if (
      activeAccordion === "recipient-details" ||
      activeAccordion === "passenger-info"
    ) {
      return;
    }
    ...
    if (activitySelect !== null && activeAccordion !== null) {
      ...
      if (
        shopifyStartAtVoucher &&
        selectedVoucherType &&
        !shopifyVoucherForcedRef.current
      ) {
        shopifyVoucherForcedRef.current = true;
        if (
          activeAccordion !== "voucher-type" &&
          activeAccordion !== "live-availability"
        ) {
      ...
      if (
        shouldForceVoucherType &&
        (activeAccordion === null || activeAccordion === undefined) &&
        !selectedVoucherType &&
        activeAccordion !== "live-availability"
      ) {
        setActiveAccordion("voucher-type");
      ...
      const isVoucherTypeOpenInShopifyFlow =
        activeAccordion === "voucher-type" &&
        (shopifyStartAtVoucher || shouldSkipAutoOpen);
      if (
        !sequence.includes(activeAccordion) &&
        !shopifyStartAtVoucher &&
        !shouldSkipAutoOpen &&
        !isVoucherTypeOpenInShopifyFlow
      ) {
        ...
        const hasExperienceFromShopify = (() => {
          try {
            const params = new URLSearchParams(location.search || "");
            return (
              params.get("source") === "shopify" && params.get("experience")
            );
          } catch {
            return false;
          }
        ...
        if (nextSection) {
          ...
          if (
            ...
            const hasRequiredState =
              chooseLocation && activityId && chooseFlightType?.type;
            if (hasRequiredState) {
              setActiveAccordion(nextSection);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const source = params.get("source");
      const startAt = params.get("startAt");
      const shouldStartAtVoucher =
        startAt === "voucher-type" && source === "shopify";
  ...
  useEffect(() => {
    ...
    if (
      journeyParam === "flight-voucher" ||
      journeyParam === "buy-flight-voucher" ||
      journeyParam === "flightvoucher"
    ) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const isGoogleMerchantStyleUrl =
        source !== "shopify" && (startAt === "voucher-type" || !!voucherTitle);
  ...
  useEffect(() => {
    try {
      ...
      if (!derivedExperience && qpVoucherTitle) {
        ...
        if (
          titleLower.includes("private charter") ||
      ...
      if (qpLocation) {
        ...
        (async () => {
          try {
            ...
            console.error(
              "🔵 Shopify prefill - Error fetching activityId:",
              error,
            );
          }
        })();
      ...
      if (derivedExperience || qpPassengers) {
        ...
        setTimeout(() => {
          ...
          const experienceCards = document.querySelectorAll(
            '.experience-card, [class*="experience"]',
          );
          experienceCards.forEach((card) => {
            ...
            if (
              derivedExperience === "Shared Flight" &&
              (cardText.includes("Shared") ||
                cardText.includes("Shared Flight"))
            ) {
              const clickEvent = new MouseEvent("click", {
                bubbles: true,
                cancelable: true,
              ...
              (cardText.includes("Private") ||
                cardText.includes("Private Charter"))
            ) {
              const clickEvent = new MouseEvent("click", {
                bubbles: true,
                cancelable: true,
      ...
      if (qpWeatherRefundable) {
        setPassengerData((prev) => {
          ...
          return [
            { firstName: "", lastName: "", weight: "", weatherRefund: true },
          ];
        });
      }
      ...
      if (qpVoucherTitle) {
        ...
        setTimeout(() => {
          ...
          const voucherCards = document.querySelectorAll(
            '.voucher-card, [class*="voucher"]',
          );
          voucherCards.forEach((card) => {
            const cardTitle = card
              .querySelector('h3, h4, .voucher-title, [class*="title"]')
              ?.textContent?.trim();
            ...
            if (
              cardTitle &&
              (cardTitle === decodedTitle || cardTitle === qpVoucherTitle)
            ) {
              const selectButton = card.querySelector(
                '.voucher-card-select-btn, button, [class*="select"]',
              );
              if (selectButton) {
                const clickEvent = new MouseEvent("click", {
                  bubbles: true,
      ...
      setTimeout(() => {
        ...
        if (qpStartAt === "voucher-type") {
          ...
          const checkAndOpenLiveAvailability = () => {
            ...
            const hasRequiredState =
              qpLocation && activityId && derivedExperience && qpVoucherTitle;
            const termsReady =
              voucherTermsLoadedRef.current || !voucherTermsLoading;
            const accordionsReady =
              !accordionLoadingStates.isAccordionLoading &&
            ...
            if (hasRequiredState && termsReady && accordionsReady) {
              ...
              if (isMobileChrome) {
                ...
                const fetchAndWait = async () => {
                  try {
                    ...
                    console.error(
                      "🔵 Mobile Chrome - Error fetching availabilities:",
                      error,
                    );
              ...
              const pollForReadiness = () => {
                pollAttempt++;
                const retryHasRequiredState =
                  qpLocation &&
                  activityId &&
                  derivedExperience &&
                  qpVoucherTitle;
                const retryTermsReady =
                  voucherTermsLoadedRef.current || !voucherTermsLoading;
                const retryAccordionsReady =
                  !accordionLoadingStates.isAccordionLoading &&
                  accordionLoadedRef.current;

                if (
                  retryHasRequiredState &&
                  retryTermsReady &&
                  retryAccordionsReady
                ) {
                  setIsLiveAvailabilityLoadingSync(true);
                  setActiveAccordion("live-availability");
                  setTimeout(() => {
  ...
  const scrollToPortalSection = useCallback(
    (sectionId) => {
  ...
  const firePurchaseTrackingFromConversionData = useCallback(
    (conversionData) => {
      ...
      if (session_id && type) {
        ...
        const createFromSession = async () => {
          try {
            ...
            if (response.data.success) {
              ...
              if (
                ...
                try {
                  ...
                  if (
                    response.data.voucher_type === "Buy Gift" ||
                    response.data.voucher_type === "Gift Voucher"
                  ) {
                  ...
                  if (
                    type === "booking" &&
                    response.data.voucher_type === "Book Flight" &&
                    !finalVoucherCode
                  ) {
                    try {
                      ...
                      const voucherCodeResponse = await axios.post(
                        `${API_BASE_URL}/api/generate-voucher-code`,
                        {
                          flight_category: flightCategory,
                          customer_name:
                            response.data.customer_name || "Unknown Customer",
                      ...
                      if (voucherCodeResponse.data.success) {
                        finalVoucherCode =
                          voucherCodeResponse.data.voucher_code;
                      } else {
                        console.error(
                          "Failed to generate Book Flight voucher code on frontend:",
                          voucherCodeResponse.data.message,
                        );
                      ...
                      console.error(
                        "Error generating Book Flight voucher code on frontend:",
                        frontendVoucherError,
                      );
                      // If still no voucher code, generate a simple one for display
                      if (!finalVoucherCode) {
                        const timestamp = Date.now().toString().slice(-6);
              ...
              const isFlightVoucher =
                ...
                (response.data.voucher_type === "Buy Flight Voucher" ||
                  ...
                  (response.data.book_flight &&
                    response.data.book_flight
                      .toLowerCase()
                      .includes("flight voucher")));
              ...
              const experienceType = (
                response.data.experience_type ||
                response.data.chooseFlightType?.type ||
                ""
              )
                .toLowerCase()
                .includes("private")
                ...
                : "shared";
              const productType =
                response.data.voucher_type_detail ||
                response.data.voucher_type ||
                "";
              const userData =
                ...
                (response.data.customer_email
                  ? {
                      email: String(response.data.customer_email)
                        .trim()
                        .toLowerCase(),
                      ...(response.data.customer_phone
                        ? {
                            phone_number: String(response.data.customer_phone)
                              .trim()
                              .replace(/\s+/g, ""),
                          }
                        : {}),
              ...
              console.warn(
                "[EmailDebug] Backend did not return success; email may not have been triggered",
                {
                  type,
                  activitySelect,
              ...
              console.warn(
                "Payment succeeded but backend did not return success. Skipping alert because entry is already created.",
              );
            }
          } catch (error) {
              ...
              for (let i = 0; i < 8; i++) {
                ...
                if (s.data?.processed) {
                  if (s.data?.conversion_data) {
                    firePurchaseTrackingFromConversionData(
                      s.data.conversion_data,
                    );
                  }

                  localStorage.setItem(
                    `fab_payment_processed_${session_id}`,
                    "1",
                  );
                  return;
                }
            ...
            console.error(
              "[EmailDebug] Error creating from session; email likely not sent",
              {
                session_id,
                type,
                activitySelect,
      ...
      <div className="customer-portal-page">
        ...
        <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto" }}>
          ...
          >
            ...
            <p>
              Here you can find all the details about your upcoming balloon
              flight.
            </p>
            <p>
              <strong>Booking Reference:</strong>{" "}
              {portalBookingData.booking_reference || portalBookingData.id}
            </p>
            <p>
              <strong>Flight Date:</strong>{" "}
              {portalBookingData.flight_date
                ? new Date(portalBookingData.flight_date).toLocaleString()
                : "Not Scheduled"}
            </p>
            <p>
              <strong>Location:</strong> {portalBookingData.location || "TBD"}
            </p>
            <p>
              <strong>Status:</strong> {portalBookingData.status || "Open"}
            </p>
          </section>
          ...
          >
            ...
            <p>
              <strong>Passengers:</strong> {portalBookingData.pax || 1}
            </p>
            <p>
              <strong>Experience:</strong>{" "}
              {portalBookingData.flight_type ||
                portalBookingData.experience ||
                "N/A"}
            </p>
            <p>
              <strong>Name:</strong> {portalBookingData.name || "N/A"}
            </p>
            <p>
              <strong>Email:</strong> {portalBookingData.email || "N/A"}
            </p>
            <p>
              <strong>Phone:</strong> {portalBookingData.phone || "N/A"}
            </p>
            {portalBookingData.voucher_code && (
              <p>
                <strong>Voucher Code:</strong> {portalBookingData.voucher_code}
              </p>
            )}
            {portalBookingData.expires && (
              <p>
                <strong>Expires:</strong>{" "}
                {new Date(portalBookingData.expires).toLocaleDateString()}
              </p>
            )}
          </section>
          ...
          >
            ...
            <p>
              Check for other available dates and times to reschedule your
              flight.
            </p>
            <div
              style={{
            ...
            requestAnimationFrame(() => {
              ...
              let target =
                ...
                document.querySelector(
                  `[data-accordion-id="${nextSectionId}"]`,
                );
              if (!target) {
                const map = {
                ...
                if (label) {
                  const btn = Array.from(
                    document.querySelectorAll("button.accordion"),
      ...
      >
        ...
        <Container>
          <div className="main-content">
            ...
            {shouldShowManualBookingBanner && (
              <div
                style={{
                  ...
                  padding: "14px 16px",
                  borderRadius: "12px",
                  background: hasInvalidManualBookingLink
                    ? "#fff1f2"
                    : "#eff6ff",
                  border: hasInvalidManualBookingLink
                    ? "1px solid #fecdd3"
                    : "1px solid #bfdbfe",
              ...
              >
                <strong style={{ display: "block", marginBottom: "4px" }}>
                  {hasInvalidManualBookingLink
                    ? isDedicatedManualBookingFlow
                      ? `${manualBookingRouteProfile?.label || "Manual booking"} link invalid`
                      : "Manual booking link invalid"
                    : manualBookingRouteProfile?.showGuide
                      ? "Hot Air Balloon Booking Guide"
                      : manualBookingRouteProfile?.label ||
                        "Manual booking mode"}
                </strong>
                {hasInvalidManualBookingLink ? (
                  <div style={{ fontSize: "14px", lineHeight: 1.5 }}>
                    This link is missing its admin authorisation token. Reopen
                    the flow from the admin system.
                  </div>
                ) : manualBookingRouteProfile?.showGuide ? (
                  <div style={{ fontSize: "14px", lineHeight: 1.5 }}>
                                  ...
                                  >
                                    ...
                                    >
                                      {item.items.map((subItem) => (
                                        <li
                                          key={`${section.title}-${item.title}-${subItem}`}
            ...
            <div
              className="main_booking"
              id="portal-booking"
              style={{ scrollMarginTop: isMobile ? "90px" : "140px" }}
            >
              <div className="booking_data">
                <div
                  className={`accodien ${isMobile ? "mobile-optimized" : ""}`}
                >
                  ...
                  <div
                    style={{
                      opacity:
                        activitySelect === null || isDedicatedFlowLocked
                          ? "0.5"
                          : "1",
                      pointerEvents:
                        activitySelect === null || isDedicatedFlowLocked
                      ...
                      <>
                        {!shouldHideLocationStep && (
                          <LocationSection
                            ...
                            onSectionCompletion={handleSectionCompletion}
                            isDisabled={
                              !getAccordionState("location").isEnabled
                            }
                          />
                        )}
                        {!shouldHideExperienceStep && (
                          <ExperienceSection
                            ...
                            onSectionCompletion={handleSectionCompletion}
                            isDisabled={
                              !getAccordionState("experience").isEnabled
                            }
                          />
                        )}
                        {(!shouldHideVoucherTypeStep ||
                          isDedicatedSelectionHidden) && (
                          <div
                            style={
                              shouldHideVoucherTypeStep
                                ? { display: "none" }
                            ...
                            aria-hidden={
                              shouldHideVoucherTypeStep ? "true" : undefined
                            }
                          >
                            <VoucherType
                              activeAccordion={activeAccordion}
                              ...
                              isDisabled={
                                shouldHideVoucherTypeStep
                                  ? true
                                  : !getAccordionState("voucher-type").isEnabled
                              }
                              seasonSaver={seasonSaver}
                              setSeasonSaver={setSeasonSaver}
                              hiddenVoucherTitles={
                                isDedicatedManualBookingFlow
                                  ? hiddenVoucherTitlesForDedicatedFlow
                                  : []
                              }
                              privateCharterPassengerOptions={
                                dedicatedBookingDefaults?.privateCharterPassengerOptions
                              }
                              ...
                              autoAdvanceToSectionId={
                                shouldAutoAdvanceVoucherSelection
                                  ? "live-availability"
                                  : null
                              }
                        ...
                        <LiveAvailabilitySection
                          ...
                          onLoadingStateChange={
                            setIsLiveAvailabilityLoadingSync
                          }
                          onCurrentDateChange={handleCalendarDateChange}
                          isDisabled={
                            !getAccordionState("live-availability").isEnabled
                          }
                        />
                        ...
                        <PassengerInfo
                          ...
                          onSectionCompletion={handleSectionCompletion}
                          isDisabled={
                            !getAccordionState("passenger-info").isEnabled
                          }
                        ...
                        {!shouldHideAddOns && (
                          <>
                            <AddOnsSection
                              ...
                              onSectionCompletion={handleSectionCompletion}
                              isDisabled={
                                !getAccordionState("add-on").isEnabled
                              }
                            />
                          </>
                        )}
                      ...
                      <>
                        ...
                        {activitySelect === "Buy Gift" && (
                          <EnterRecipientDetails
                            ...
                            onSectionCompletion={handleSectionCompletion}
                            isDisabled={
                              !getAccordionState("recipient-details").isEnabled
                            }
                          />
                        )}
            ...
            >
              ...
              >
                ...
                {(() => {
                  ...
                  if (
                    paymentSuccessData.voucherCodes &&
                    Array.isArray(paymentSuccessData.voucherCodes) &&
                    paymentSuccessData.voucherCodes.length > 1
                  ) {
                    ...
                    const voucherCode =
                      paymentSuccessData.voucherCode ||
              ...
              {paymentSuccessData.manualBooking &&
                paymentSuccessData.quotedTotal !== null &&
                paymentSuccessData.quotedTotal !== undefined && (
                  <div
                    style={{
