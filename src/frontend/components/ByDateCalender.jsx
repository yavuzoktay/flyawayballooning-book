    ...
    while (date <= endDate) {
      ...
      days.push(
        ...
        >
          ...
          {!isPastDate && (
            <div className="activity">Shop 5 Activities</div>
          )}{" "}
          {/* Show activities only for current or future dates */}
        </div>,
      );
      date = addDays(date, 1);
    }
  ...
  return (
    <div className="calendar">
      <div className="header">
        <div className="calender-prev calender-arrow" onClick={handlePrevMonth}>
          <ArrowBackIosIcon />
        </div>
        <h2>{format(currentDate, "MMMM yyyy")}</h2>
        <div className="calender-next calender-arrow" onClick={handleNextMonth}>
          <ArrowForwardIosIcon />
        </div>
      </div>
