...
const SHORT_NOTICE_QUESTION_TEXT =
  "Would you like to receive short notice flight availability?";
...
const isShortNoticeQuestion = (value = "") =>
  normalizeQuestionText(value).includes(
    normalizeQuestionText(SHORT_NOTICE_QUESTION_TEXT),
  );

const AdditionalInfo = forwardRef(
  (
    {
      isGiftVoucher,
      isRedeemVoucher,
    ...
    useEffect(() => {
      const fetchAdditionalInfoQuestions = async () => {
        try {
          ...
          const response = await fetch(
            `${config.API_BASE_URL}/api/additional-information-questions`,
          );
          if (response.ok) {
          ...
          console.error(
            "Error fetching additional information questions:",
            error,
          );
        } finally {
      ...
      location,
      flightType,
    ]); // Refetch when journey type, location, or flight type changes
        ...
        .filter((question) => {
          ...
          if (question.journey_types) {
            try {
              ...
              console.warn(
                `❌ Error parsing journey_types for question "${question.question_text}":`,
                error,
              );
              return false;
            }
          ...
          if (question.locations && currentLocation) {
            try {
              ...
              console.warn(
                `❌ Error parsing locations for question "${question.question_text}":`,
                error,
              );
              locationMatch = false;
            }
            ...
            try {
              ...
              if (currentFlightType && typeof currentFlightType === "string") {
                if (currentFlightType.toLowerCase().includes("shared")) {
                  ...
                  currentFlightType.toLowerCase().includes("private")
                ) {
              ...
              if (mappedFlightType) {
                experienceTypeMatch =
                  experienceTypes.includes(mappedFlightType);
              } else {
              ...
              console.warn(
                `❌ Error parsing experience_types for question "${question.question_text}":`,
                error,
              );
    ...
    const shortNoticeFieldName = shortNoticeQuestion
      ? `question_${shortNoticeQuestion.id}`
      : null;
    const shortNoticeAnswer = shortNoticeFieldName
      ? additionalInfo?.[shortNoticeFieldName]
      : undefined;
    ...
    useEffect(() => {
      try {
        ...
        console.warn(
          "Failed to set required keys for Additional Information",
          e,
        );
      }
    ...
    useEffect(() => {
      setAdditionalInfo((prev) => {
        ...
        const shouldOptOut =
          (shortNoticeAnswer || "").toString().trim().toLowerCase() === "no";
    ...
    useEffect(() => {
      ...
      const isValid =
        requiredQuestions.length > 0 ? allRequiredFilled : hasOptionalFilled;
                          ...
                          try {
                            ...
                            if (Array.isArray(parsedOptions)) {
                              return parsedOptions.map((option, index) => (
                                <label
                                  key={index}
                                  className={`block mb-2 ${isMobile ? "flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer" : ""}`}
                                >
                                  <input
                                    type="radio"
                                    name={fieldName}
                                    ...
                                    onChange={handleChange}
                                    checked={
                                      additionalInfo[fieldName] === option
                                    }
                                    required={isRequired}
                              ...
                              return parsedOptions.map((option, index) => (
                                ...
                                >
                                  <input
                                    ...
                                    value={option}
                                    onChange={(e) => {
                                      const currentValues = additionalInfo[
                                        fieldName
                                      ]
