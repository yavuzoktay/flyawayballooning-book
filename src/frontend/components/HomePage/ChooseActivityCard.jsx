...
import {
  trackFunnelStart,
  activityToFunnelType,
} from "../../../utils/googleAdsTracking";

const ChooseActivityCard = ({
  activitySelect,
  setActivitySelect,
  onVoucherSubmit,
  voucherStatus,
  ...
  const activityCardHeight = isMobile
    ? mobileActivityCardHeight
    : desktopActivityCardHeight;
  ...
  const selectActivityData = [
    {
      value: 0,
      label: "Book Flight",
      displayLabel: "Book Flight Date",
      subText: "",
    },
    {
      value: 1,
      label: "Flight Voucher",
      displayLabel: "Buy Flight Voucher",
      subText: "",
    },
    {
      value: 3,
      label: "Buy Gift",
      displayLabel: "Buy Gift Voucher",
      subText: "",
    },
  ...
  const visibleActivityCards =
    Array.isArray(allowedActivities) && allowedActivities.length > 0
      ? selectActivityData.filter((item) =>
          allowedActivities.includes(item.label),
        )
      : selectActivityData;
  ...
  return (
    <>
      ...
      <div
        className="tab_box choose-activity-tab-box"
        style={{
          display: "grid",
          gridTemplateColumns:
            visibleActivityCards.length === 1
              ? "1fr"
              ...
              {item.label === "Redeem Voucher" ? (
                ...
                >
                  <input
                    ...
                    value={item.label}
                    checked={activitySelect === item.label}
                    onChange={() => handleActivitySelect(item.label)}
                    style={{ display: "none" }}
                  />
                  <div
